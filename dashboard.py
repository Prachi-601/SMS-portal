...
def get_student_class_by_id(student_id):
    try:
...
def student_dashboard(name,student_id):
    print(f"\n📋 Dashboard for {name}")
    ...
    submitted = sum(1 for s in submissions if str(s.get("student_id")) == str(student_id))
    ...
    marks = [m for m in all_marks if str(m.get("student_id")) == str(student_id)]
    print("📊 Test Marks:")
    if marks:
        for m in marks:
    ...
    student_class = get_student_class_by_id(student_id)
    if student_class:
